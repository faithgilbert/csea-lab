function [condvec4ERP,condvec_full picvec, statvec_aro, statvec_val] = getcon_MyAPS2(filepath)
condvec = []; 
picvec = [];
trialnums  = []; 
unparsedratings = []; 
...
a = 1;
 ...
 while a > 0
    ...
    if sum(contains(p)) > 0, concode = 1
    elseif sum(contains(n, 'IAPS')) > 0, concode = 2; 
    elseif sum(contains(u, 'IAPS')) > 0, concode = 3;
    end
    
    picvec = [ picvec; picname];
    
    condvec_full = [condvec; AIcondition+concode]; 
